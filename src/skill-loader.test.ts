...
This is the content that should not be loaded`
    ...
    it('should throw error for non-existent skill', async () => {
      ...
      await expect(skillLoader.getSkillMetadata('non-existent')).rejects.toThrow(
        'Skill "non-existent" not found'
      );
    });
    ...
    it('should throw error for corrupted skill file', async () => {
      ...
      await expect(skillLoader.getSkillMetadata('corrupted-skill')).rejects.toThrow(
        'Failed to load metadata for skill "corrupted-skill"'
      );
    });
  });
